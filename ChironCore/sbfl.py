...
class GeneticAlgo():
    ...
    def execute(self):
        ...
        for itr in range(1,self.ngen + 1):
            ...
            if self.verbose:
                print("Iteration : {}, Best fit-score : {}, |pop|: {}, |Test-Suite| :{}".format(itr,
                            self.population[0].fitness, len(self.population), len(self.population[0].individual)))
...
class SBFLAnalysis(ConcreteInterpreter):
    ...
    def generateSpectrum(self, orcl, timeLimit):
        # run correct and buggy program to get error vector [Fault Oracle]
        spectrum = np.zeros((len(orcl.reduced_tests),
                                len(orcl.ir2) + 1), dtype = 'int')
        executer = Executor()

        for i,test in enumerate(orcl.reduced_tests):
            _, ir1_trltl_pos = executer.execute(ir = orcl.ir1,
                                                    end=(time.time() + timeLimit), inputList=test)
            cov, ir2_trltl_pos = executer.execute(ir = orcl.ir2,
                                                    end=(time.time() + timeLimit), inputList=test)
            spectrum[i, cov] = 1
...
def testsuiteGenerator(irhandler1, irhandler2, inputVars = [], Ntests = 10,
                       timeLimit = 10, popsize = 50, cxpb = 0.5, mutpb = 0.5, ngen = 50, verbose = True):
    ...
    original_tests = sbfl_object.generateTests(inputVars = inputVars,
                                                total_tests = Ntests)
    ActivityMatrix = sbfl_object.generateSpectrum(original_tests)
    ...
    ActivityMatrix = ActivityMatrix.tolist()

    # optimize test-suite and get the best indivudual return by GA.
    genOpt = GeneticAlgo(spectrum=ActivityMatrix, popsize=popsize,
                            cxpb=cxpb, mutpb=mutpb, ngen=ngen, verbose=verbose)
    genOpt.execute()
    ...
    orcl = FaultOrcale(irhandler1.ir, irhandler2.ir, reduced_tests)
    spectrum = sbfl_object.generateSpectrum(FaultOrcale)

    return original_test_suites, original_tests, reduced_act_mat, reduced_tests, spectrum
